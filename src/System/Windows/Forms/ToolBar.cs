...
namespace System.Windows.Forms
{
    ...
    public class ToolBar : Control
    {
        ...
        public Size ButtonSize
        {
            get
            {
                if (buttonSize.IsEmpty)
                {
                    ...
                    {
                        int result = unchecked((int)(long)User32.SendMessageW(Handle, NativeMethods.TB_GETBUTTONSIZE, IntPtr.Zero, IntPtr.Zero));
                        if (result > 0)
                        {
        ...
        internal int PreferredHeight
        {
            get
            {
                ...
                else
                {
                    ...
                    User32.SendMessageW(this, NativeMethods.TB_GETRECT, (IntPtr)firstVisible, ref rect);
                ...
                {
                    height *= unchecked((int)(long)User32.SendMessageW(this, NativeMethods.TB_GETROWS, IntPtr.Zero, IntPtr.Zero));
                }
            ...
            get
            {
                ...
                if (maxWidth == -1)
                {
                    ...
                    else
                    {
                        ...
                        {
                            User32.SendMessageW(this, NativeMethods.TB_GETRECT, IntPtr.Zero, ref rect);
        ...
        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                ...
                lock (this)
                {
                    ...
                    try
                    {
                        this.SetState(STATE_DISPOSING, true);
                    ...
                    {
                        this.SetState(STATE_DISPOSING, currentDisposing);
                    }
                }
            }
        ...
        internal void InternalSetButton(int index, ToolBarButton value, bool recreate, bool updateText)
        {
            ...
            if (IsHandleCreated)
            {
                ...
                else
                {
                    ...
                    //
                    User32.SendMessageW(this, NativeMethods.TB_AUTOSIZE, IntPtr.Zero, IntPtr.Zero);
        ...
        protected override void OnHandleCreated(EventArgs e)
        {
            ...
            //
            User32.SendMessageW(this, NativeMethods.TB_BUTTONSTRUCTSIZE, (IntPtr)Marshal.SizeOf<NativeMethods.TBBUTTON>(), IntPtr.Zero);

            // set up some extra goo
            //
            if (DropDownArrows)
            {
                User32.SendMessageW(this, NativeMethods.TB_SETEXTENDEDSTYLE, IntPtr.Zero, (IntPtr)NativeMethods.TBSTYLE_EX_DRAWDDARROWS);
            }
            ...
            if (imageList != null)
            {
                User32.SendMessageW(this, NativeMethods.TB_SETIMAGELIST, IntPtr.Zero, imageList.Handle);
            }
                ...
                try
                {
                    ...
                    for (int x = 0; x < buttonCount; x++)
                    {
                        if (buttons[x].Text.Length > 0)
                        {
                            ...
                            buttons[x].stringIndex = User32.SendMessageW(this, NativeMethods.TB_ADDSTRING, IntPtr.Zero, addString);
                        }
                        else
                        {
                    ...
                    User32.SendMessageW(this, NativeMethods.TB_ADDBUTTONS, (IntPtr)count, ptbbuttons);
        ...
        {
            User32.SendMessageW(this, NativeMethods.TB_SETBUTTONSIZE, IntPtr.Zero, NativeMethods.Util.MAKELPARAM((int)(buttonSize.Width * currentScaleDX), (int)(buttonSize.Height * currentScaleDY)));
        }
        ...
        {
            IntPtr toolTipHandle = (IntPtr)typeof(ToolTip).GetProperty("Handle", BindingFlags.Instance | BindingFlags.NonPublic).GetValue(toolTip);
            UnsafeNativeMethods.SendMessage(new HandleRef(this, Handle), NativeMethods.TB_SETTOOLTIPS, new HandleRef(toolTip, toolTipHandle), 0);
        }
        ...
        private void WmNotifyDropDown(ref Message m)
        {
            ...
            if (menu != null)
            {
                ...
                User32.SendMessageW(this, NativeMethods.TB_GETRECT, (IntPtr)nmTB.iItem, ref rc);
        ...
        public class ToolBarButtonCollection : IList
        {
            ...
            public void Clear()
            {
                ...
                for (int x = owner.buttonCount; x > 0; x--)
                {
                    ...
                    {
                        User32.SendMessageW(owner, NativeMethods.TB_DELETEBUTTON, (IntPtr)(x - 1), IntPtr.Zero);
                    }
                    owner.RemoveAt(x - 1);
                }
            ...
            public void RemoveAt(int index)
            {
                ...
                if (owner.IsHandleCreated)
                {
                    User32.SendMessageW(owner, NativeMethods.TB_DELETEBUTTON, (IntPtr)index, IntPtr.Zero);
                }
            ...
            {
                return new ArraySubsetEnumerator(owner.buttons, owner.buttonCount);
            }
        }
